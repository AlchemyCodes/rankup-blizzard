...
public class PlayersDAO {
    ...
    public void initializeDatabase() {
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            String sql = "CREATE TABLE IF NOT EXISTS monsters_users (" +
        ...
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    List<String> rewards = rewardsJson != null ? gson.fromJson(rewardsJson, new TypeToken<List<String>>() {}.getType()) : new ArrayList<>();
                    return new PlayersData(
                            resultSet.getString("uuid"),
                            resultSet.getString("nickname"),
                            resultSet.getInt("killed_monsters"),
    ...
    public void createPlayerData(PlayersData playerData) throws SQLException {
        String sql = "INSERT INTO monsters_users (uuid, nickname, killed_monsters, monsters_limit, rewards) VALUES (?, ?, ?, ?, ?)";
        executeUpdate(sql, statement -> {
            try {
                statement.setString(1, playerData.getUuid());
                statement.setString(2, playerData.getNickname());
                statement.setInt(3, playerData.getKilledMonsters());
                statement.setInt(4, playerData.getMonstersLimit());
                statement.setString(5, gson.toJson(playerData.getRewards()));
            } catch (SQLException e) {
    ...
    public void deletePlayerData(PlayersData playerData) throws SQLException {
        String sql = "DELETE FROM monsters_users WHERE uuid = ?";
        executeUpdate(sql, statement -> {
            try {
                statement.setString(1, playerData.getUuid());
            } catch (SQLException e) {
    ...
    public void updatePlayerData(PlayersData playerData) throws SQLException {
        String sql = "UPDATE monsters_users SET nickname = ?, killed_monsters = ?, monsters_limit = ?, rewards = ? WHERE uuid = ?";
        executeUpdate(sql, statement -> {
            try {
                statement.setString(1, playerData.getNickname());
                statement.setInt(2, playerData.getKilledMonsters());
