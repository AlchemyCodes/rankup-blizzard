...
public class PlantationManager {
    ...
    public void transformWithFriend(Player player, Player friend, int radius) {
        Location centerLocation = LocationUtils.getCenterLocation();
        if (centerLocation == null) return;
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (Block block : batch) {
                    ...
                    BlockManager.placePlantation(player, blockPosition);
                    sendFriendPlantationPacket(player, friend, block);
                }
    ...
    private void sendFriendPlantationPacket(Player player, Player friend, Block block) {
        try {
            Block blockAbove = block.getRelative(0, 1, 0);
            if (blockAbove.getType() != Material.AIR) {
                return;
            ...
            BlockData blockData;
            String friendPlantationType = AreaManager.getInstance().getAreaPlantation(friend);

            if (friendPlantationType.equalsIgnoreCase("BEETROOTS")) {
                blockData = Bukkit.createBlockData("minecraft:" + friendPlantationType.toLowerCase() + "[age=3]");
            } else {
                blockData = Bukkit.createBlockData("minecraft:" + friendPlantationType.toLowerCase() + "[age=7]");
            }
