package blizzard.development.currencies.commands.currencies.limits.subcommands;
...
public class LimitsExchangeCommand extends BaseCommand {
    ...
    public void onPayCommand(CommandSender sender, String nickname, String amount) {
        ...
        if (!impl.Config.getBoolean("currencies.can-pay.spawners-limit")) {
        ...
        if (player.equals(target)) {
            sender.sendActionBar(TextAPI.parse("§c§lEI! §cVocê não pode enviar limites para si mesmo."));
            return;
        }
        ...
        try {
            ...
            if (bal < doubleAmount) {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cVocê não possui a quantia necessária de limites para realizar essa transação."));
                return;
            }
            if (doubleAmount <= 0) {
            ...
            if (api.transferBalance(player, target, currency, doubleAmount)) {
                List<String> playerMessages = Arrays.asList(
                        "",
                        "§3§lTransferência realizada!",
                        "§3Você pagou ∞" + "§l" + format.formatNumber(doubleAmount) + "§3 limites para o jogador " + target.getName() + ".",
                        ""
                );

                List<String> targetMessages = Arrays.asList(
                        "",
                        "§3§lTransferência recebida",
                        "§3Você recebeu ∞" + "§l" + format.formatNumber(doubleAmount) + "§3 limites do jogador " + player.getName() + ".",
                        ""
                );
            ...
            if (api.transferBalance(player, target, currency, doubleFormatted)) {
                List<String> playerMessages = Arrays.asList(
                        "",
                        "§3§lTransferência realizada!",
                        "§3Você pagou ∞" + "§l" + format.formatNumber(doubleFormatted) + "§3 limites para o jogador " + target.getName() + ".",
                        ""
                );
                ...
                List<String> targetMessages = Arrays.asList(
                        "",
                        "§3§lTransferência recebida!",
                        "§3Você recebeu ∞" + "§l" + format.formatNumber(doubleFormatted) + "§3 limites do jogador " + player.getName() + ".",
                        ""
                );
    ...
    public void onSetCommand(CommandSender sender, String nickname, String amount) {
        ...
        try {
            ...
            if (api.setBalance(target, currency, doubleAmount)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê setou " + format.formatNumber(doubleAmount) + " limites para o jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        } catch (NumberFormatException ex) {
            ...
            if (api.setBalance(target, currency, doubleFormatted)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê setou " + format.formatNumber(doubleFormatted) + " limites para o jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        }
    ...
    public void onAddCommand(CommandSender sender, String nickname, String amount) {
        ...
        try {
            ...
            if (api.addBalance(target, currency, doubleAmount)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê adicionou " + format.formatNumber(doubleAmount) + " limites para o jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        } catch (NumberFormatException ex) {
            ...
            if (api.addBalance(target, currency, doubleFormatted)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê adicionou " + format.formatNumber(doubleFormatted) + " limites para o jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        }
    ...
    public void onRemoveCommand(CommandSender sender, String nickname, String amount) {
        ...
        try {
            ...
            if (api.removeBalance(target, currency, doubleAmount)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê removeu " + format.formatNumber(doubleAmount) + " limites do jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        } catch (NumberFormatException ex) {
            ...
            if (api.removeBalance(target, currency, doubleFormatted)) {
                sender.sendActionBar(TextAPI.parse("§a§lYAY! §aVocê removeu " + format.formatNumber(doubleFormatted) + " limites do jogador " + target.getName()));
            } else {
                sender.sendActionBar(TextAPI.parse("§c§lEI! §cOcorreu um erro ao concluir a transação."));
            }
        }
