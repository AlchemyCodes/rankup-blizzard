package blizzard.development.spawners.inventories.slaughterhouses.friends;
...
import blizzard.development.spawners.inventories.slaughterhouses.friends.items.FriendsItems;
...
public class FriendsInventory {
    ...
    public void open(Player player, String id) {
        ...
        ChestGui inventory = new ChestGui(4, "§8Amigos");

        final FriendsItems items = FriendsItems.getInstance();

        StaticPane pane = new StaticPane(0, 0, 9, 4);

        List<String> friends = getters.getSlaughterhouseFriends(id);

        String[] slots = {"11", "12", "13", "14", "15"};

        for (int i = 0; i < slots.length; i++) {
            if (i < friends.size()) {
                String slaughterhouseFriend = friends.get(i);

                GuiItem slaughterhousesItem = new GuiItem(items.friends(slaughterhouseFriend, i), event -> {
                    removeFriend(player, id, slaughterhouseFriend);
                    event.setCancelled(true);
                });
                pane.addItem(slaughterhousesItem, Slot.fromIndex(Integer.parseInt(slots[i])));
            } else {
        ...
        pane.addItem(manageItem, Slot.fromIndex(22));
    ...
    public void removeFriend(Player player, String id, String friend) {
        if (!getters.getSlaughterhouseFriends(id).contains(friend)) {
            player.sendActionBar("§c§lEI! §cEste jogador não está mais na lista de amigos.");
            return;
        }

        player.sendActionBar("§a§lYAY! §aO jogador §7" + friend + "§a foi removido como amigo do abatedouro.");
        player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_YES, 1.0f, 1.0f);
        setters.removeSlaughterhouseFriend(id, List.of(friend));
        open(player, id);
    }

    public void addFriend(String id, Player player) {
        if (getters.getSlaughterhouseFriends(id).size() >= getters.getSlaughterhouseFriendsLimit(id)) {
        ...
        SlaughterhousesFriendsListener.addPendingInvite(player, id);
